...
|---|---|---|---|---|
| Hybrid-WR + Batterie | Solis S6 Hybrid 50 kW | Datalogger mit Netzwerkschnittstelle | profiles/solis_s6_hybrid.lua | Register aus ESINV-33000ID v3.4 uebernommen; Datalogger-TCP-Durchleitung UNVERIFIZIERT |
...
1. Solis-Datalogger: genauer Typ? Reicht er Modbus TCP (Port 502)
   transparent durch? NICHT jeder Solis-Stick kann das - sonst
   Alternative: RS485 direkt auf den RUTX11 (hat RS485 onboard,
   RTU-Protokoll identisch).
2. Solis Slave-Adresse (Default 1?) und Erreichbarkeit ueber den
   Datalogger testen (test.html, read-only).
3. Kaco NX3: SunSpec-Kennung 'SunS' bei 40000 lesen, Modellkette
   scannen, W/Hz/SF-Adressen bestaetigen, Unit-ID klaeren.
4. Janitza UMG 604: Psum 19026 (f32be) und Word-Order am Geraet
   verifizieren; Unit-ID klaeren.
5. Router fuer Hebauer: eigenes GitHub-Token, Bootstrap nach
